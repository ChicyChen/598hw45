...
dataset_5 = InductionAR(num_examples, tokenizer, 5, n_ctx=n_ctx, seed=seed, train_split=0.8)
# train_loader = DataLoader(dataset_5.train, batch_size=batch_size, shuffle=True)
# test_loader = DataLoader(dataset_5.test, batch_size=batch_size, shuffle=True)

dataset_lists = [dataset, dataset_5]
roots = ["ckpt_p1_1", "ckpt_p1_2"]
data_idx = 0
for datas in dataset_lists:
    root = roots[data_idx]
    ...
    models = [attn_model, rope_model, nope_model, mamba_model, hyba_model, hybb_model] # add more models here when you have more models
    model_idx = 0
    plt.figure()
    for model in models:
        model.train()
        ...
        EPOCH = num_epochs
        PATH = os.path.join(root, model_names[model_idx]+".pt")
        torch.save({
                    'epoch': EPOCH,
                    'model_state_dict': models[model_idx].state_dict(),
                    'optimizer_state_dict': optimizer.state_dict(),
                    }, PATH)
        model_idx += 1

        
        plt.plot(range(len(test_list)), test_list, label = model_name)
    ...
    plt.ylabel('Acc')
    plt.legend()
    plt.savefig(os.path.join(root, 'AR_Acc_Itr.png'))
    data_idx += 1
